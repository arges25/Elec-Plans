...
export function ZoomControls({ className = '' }: { className?: string }) {
  ...
  return (
    <div
      className={`flex flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white/95 shadow-lg backdrop-blur ${className}`}
      role="group"
      aria-label="Zoom"
    >
      ...
      <button
        type="button"
        className="min-h-9 border-y border-gray-100 px-1 text-[11px] font-bold tabular-nums text-gray-700 hover:bg-gray-100"
        aria-label="Zoom 100 %"
